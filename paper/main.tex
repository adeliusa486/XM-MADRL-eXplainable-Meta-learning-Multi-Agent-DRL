...
multi-agent EW benchmark with adaptive jamming, over five random seeds at an
\emph{identical} training budget with full statistical testing. Under this fair
protocol, XM-MADRL performs on par with the strongest baselines across mission
success, energy, and flight safety (zero collisions), while additionally
providing SHAP-based interpretability that the baselines lack; its decoupled
spectrum head more than doubles packet-delivery ratio over a naive graph-only
policy, and a constellation-based classifier attains $84\%$ modulation-recognition
accuracy. We report the comparison honestly, including where the proposed method
matches rather than exceeds simpler baselines and a residual anti-jamming gap.
The contribution is thus a unified, interpretable, and rigorously reproducible
framework together with the decoupled-head design insight, rather than a claim of
...
Table~\ref{tab:main} and Fig.~\ref{fig:overall} summarize the comparison at an
equal $6\times10^5$-step budget. Under this fair protocol, XM-MADRL is
\emph{statistically indistinguishable} from the strongest baseline (PPO) on
mission success ($p{=}0.91$), energy ($p{=}0.87$), and flight safety (both reach
zero collisions), and its PDR/SINR differences are not significant
(Table~\ref{tab:stats}). It does not outperform simple PPO on this benchmark, and
is modestly lower on anti-jamming---a fact we report transparently. The
off-policy MADDPG baseline records a higher raw mission count only by flying
aggressively, incurring hundreds of collisions and the worst energy use, and is
therefore not a usable policy. The practical value of XM-MADRL over the tied
baselines is not raw score but the interpretability (Section~\ref{sec:xai}) and
the decoupled-head design (Section~\ref{sec:ablation}) it adds, at no cost to
performance.
...
is on par with the on-policy baselines and far below the reckless MADDPG policy.}
\label{fig:energy}
\end{figure}

\subsection{Ablation Study}
\label{sec:ablation}
Table~\ref{tab:ablation} and Fig.~\ref{fig:abl} isolate each component. The
Transformer fusion and the local channel head are the decisive components:
removing the Transformer collapses PDR, and removing the local channel head
sharply degrades PDR and anti-jamming, directly validating the decoupled design.
The GNN and MAML have smaller marginal effects on steady-state metrics---
consistent with the fully-observable setting---contributing coordination
structure and adaptation capability rather than large score changes.
...
swarm size for both methods, as more agents cover more targets, and XM-MADRL
remains on par with the baseline at every size, indicating that its graph-based
coordination scales gracefully under the larger joint action space rather than
degrading. Communication reliability instead
...
that local RF sensing must not be over-smoothed by neighbor aggregation---a
methodological finding that transfers to any multi-agent system combining graph
communication with local sensing. Second, a candid reading of
Table~\ref{tab:main} is that on this fully-observable cooperative task, the
sophisticated architecture \emph{matches} rather than beats a well-tuned PPO:
when every agent can already sense its objective, graph coordination and
meta-adaptation add capability (information sharing, fast adaptation, and
interpretability) without a raw-score gain. We view exposing this honestly, at
equal budget, as more useful than an inflated comparison. Third, the practical
differentiator is interpretability: XM-MADRL matches the baselines while yielding
auditable SHAP rationale that opaque policies cannot, which is decisive in
mission-critical EW deployments. We expect the architectural advantages to
materialize as raw gains under stronger partial observability and
non-stationarity, a direction we outline below.

\subsection{Limitations}
The central limitation is honest: on this fully-observable benchmark XM-MADRL
matches but does not exceed simple baselines, and is modestly lower on
anti-jamming (Table~\ref{tab:stats}); its added machinery buys interpretability
and adaptation capability rather than raw score. The environment, while capturing
the salient EW dynamics, is a simulation rather than a photorealistic or
hardware-in-the-loop testbed, and---because every agent already observes its
objective---it does not stress the coordination and adaptation the architecture
targets. A task with strong partial observability (local-only sensing) and
non-stationary adversaries, where independent policies provably struggle, is the
natural setting in which these components should yield measurable gains, and is
our primary direction for future work.
...
aggregation. Evaluated fairly at equal budget, XM-MADRL matches strong DRL
baselines on mission success, energy, and safety while adding auditable SHAP
explanations they lack; we report this parity transparently rather than claiming
an unearned advantage. The lasting contributions are the unified interpretable
architecture, the decoupled-head insight, and a fully reproducible benchmark and
evaluation. Future work targets strongly partially-observable, non-stationary EW
tasks---where coordination and meta-adaptation should translate into measurable
gains---as well as hardware-in-the-loop validation.
